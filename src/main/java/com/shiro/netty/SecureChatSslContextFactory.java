...
public final class SecureChatSslContextFactory {
    ...
    /**
     * 客户端
     * 双向模式 密钥管理器和信任库都要生成 CLIENT_CONTEXT.init(有值,有值,null)
     * 单项模式 生成密钥管理器   CLIENT_CONTEXT.init(有值,null,null)
     * @param pkPath
     * @param caPath
     * @return
     */
     ...
     * 客户端
     * 双向模式 密钥管理器和信任库都要生成 CLIENT_CONTEXT.init(有值,有值,null)
     * 单项模式  生成任务库   CLIENT_CONTEXT.init(null,有值,null)
     * @param pkPath
     * @param caPath
     * @return
     */
    ...
    private static void close(InputStream in, InputStream tIN) {
        if (in != null) {
            try {
                in.close();
            } catch (IOException e) {
